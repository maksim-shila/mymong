...
export abstract class EnemyBase extends Phaser.GameObjects.Rectangle implements Enemy {
  ...
  public lives: number;
  public abstract override readonly type: EnemyType;
  public override state: EnemyState = EnemyState.ALIVE;
  ...
  public get isActive(): boolean {
    return (
      this.state !== EnemyState.READY_TO_DESTROY &&
      this.state !== EnemyState.DESTROING &&
      this.state !== EnemyState.DESTROYED
    );
  }

  public override update(delta: number, shotAreaX: MinMax, shotAreaY: MinMax): void {
    if (this.state === EnemyState.CONSTRUCTING) {
