...
import type { ShipBarrel } from './weapon/ship-weapon';
...
export class Ship extends Phaser.GameObjects.Rectangle {
  ...
  public weapon: ShipBarrel | null = null;
  ...
  private updateAngle(deltaMs: number): void {
    const deltaSeconds = deltaMs / 1000;
    const targetAngleDeg = Math.abs(this.vx) < 200 ? 0 : this.direction * MOVE_TILT_ANGLE_MAX_DEG;
