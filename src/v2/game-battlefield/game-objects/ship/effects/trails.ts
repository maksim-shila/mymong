...
import type { IDrawable } from '@core/game-objects/types';
...
export class Trails implements IDrawable {
  ...
  public draw(delta: number): void {
    this.updateSnapshots(delta, this.ship.dash.active);
    this.drawGhosts();
  }
  ...
  private updateSnapshots(delta: number, active: boolean): void {
    this.snapshotTimer.tick(delta);

    if (active && this.snapshotTimer.done) {
